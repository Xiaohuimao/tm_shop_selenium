...
class Vincent(object):
    ...
    def visit_index(self):
        ...
        #    EC.element_to_be_clickable((By.XPATH, '//*[@id="captcha"]/div[2]/div/div')))
    ...
    def analog_drag(self):
        ...
        element = self.driver.find_element_by_css_selector('.sliderIcon')
        ActionChains(self.driver).move_to_element(element).perform()
        time.sleep(3)

        # 刷新一下极验图片
        element = self.driver.find_element_by_xpath('//*[@id="captcha"]/div[1]')
        element.click()
        time.sleep(1)

        # 获取图片地址和位置坐标列表
        cut_image_url, cut_location = self.get_image_url('/html/body/div/canvas[2]')
        full_image_url, full_location = self.get_image_url('//*[@id="captcha"]/canvas[1]')
        ...
        # 判断是否验证成功
        s = self.driver.find_elements_by_xpath('//*[@id="captcha"]/div[1]')
        if len(s) == 0:
            print("滑动解锁失败,继续尝试")
            self.analog_drag()
        else:
    ...
    def start_move(self, distance):
        element = self.driver.find_element_by_xpath('//*[@id="captcha"]/div[2]/div/div')
